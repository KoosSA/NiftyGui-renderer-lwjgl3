...
public class Lwjgl3MouseCursor implements MouseCursor {
	...
	private static long normal_cursor = 0;
	...
	public Lwjgl3MouseCursor(final long glfwWindow, @Nonnull final String cursorImageFilename, final int hotspotX,
		...
		try {
			...
			imageData.rewind();
			GLFWImage image = new GLFWImage(BufferUtils.createByteBuffer(16));
	...
	public void disable() {
		if (normal_cursor == 0) {
			normal_cursor = glfwCreateStandardCursor(GLFW_ARROW_CURSOR);
		}
		glfwSetCursor(glfwWindow, normal_cursor);
	}
}
